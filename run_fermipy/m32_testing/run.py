...
gta = GTAnalysis('config_M32.yaml', loglevel=loggerLevel)
...
resid = gta.residmap('outputs/m32_prefit',model={'SpatialModel' : 'PointSource', 'Index' : 2.0})
...
resid = gta.residmap('outputs/m32_postfit',model={'SpatialModel' : 'PointSource', 'Index' : 2.0})
fig = plt.figure(figsize=(14,6))
ROIPlotter(resid['sigma'],roi=gta.roi).plot(vmin=-5,vmax=5,levels=[-5,-3,3,5],subplot=121,cmap='RdBu_r')
plt.gca().set_title('Significance')
ROIPlotter(resid['excess'],roi=gta.roi).plot(vmin=-100,vmax=100,subplot=122,cmap='RdBu_r')
...
tsmap_postfit = gta.tsmap('outputs/m32_postfit',model={'SpatialModel' : 'PointSource', 'Index' : 2.0})
fig = plt.figure(figsize=(14,6))
...
resid = gta.residmap('outputs/m32_newsrcs',model={'SpatialModel' : 'PointSource', 'Index' : 2.0})
fig = plt.figure(figsize=(14,6))
ROIPlotter(resid['sigma'],roi=gta.roi).plot(vmin=-5,vmax=5,levels=[-5,-3,3,5],subplot=121,cmap='RdBu_r')
plt.gca().set_title('Significance')
ROIPlotter(resid['excess'],roi=gta.roi).plot(vmin=-100,vmax=100,subplot=122,cmap='RdBu_r')
...
tsmap_newsrcs = gta.tsmap('outputs/m32_newsrcs',model={'SpatialModel' : 'PointSource', 'Index' : 2.0})
fig = plt.figure(figsize=(14,6))
...
sed_m32 = gta.sed('m32')
sed_j2300 = gta.sed('4FGL J2300.3+3136') #for comparison
gta.write_roi('outputs/fit_sed')

#inspect fit results
# E^2 x Differential flux ULs in each bin in units of MeV cm^{-2} s^{-1}
print(sed_m32['e2dnde_ul95'])
#dloglike_scan array saves likelihood in each bin
e2dnde_scan = sed_m32['norm_scan']*sed_m32['ref_e2dnde'][:,None]

for b in range(len(sed_m32['dloglike_scan'])):
	plt.figure()
	plt.plot(e2dnde_scan[b],sed_m32['dloglike_scan'][b]-np.max(sed_m32['dloglike_scan'][b]),label='Log likelihood')
	plt.gca().set_ylim(-5,1)
	plt.gca().axvline(sed_m32['e2dnde_ul95'][b],color='k',label='Flux UL')#flux UL  in this bin
	plt.gca().axhline(-2.71/2.,color='r',label='Reduction of -2.71/2 from max')
	plt.xlabel('Energy [MeV]')
	plt.ylabel('Delta Log likelihood')
	plt.title('Bin '+str(b)+' of '+str(len(sed_m32['dloglike_scan'])))
	plt.legend(loc='best')
...
xlim=[1E3,1E6]
fig.add_subplot(121)
SEDPlotter(sed_m32).plot()
plt.gca().set_ylim(ylim) 

fig.add_subplot(122)
SEDPlotter(sed_j2300).plot()
plt.gca().set_ylim(ylim) 
...
fig.add_subplot(121)
SEDPlotter(sed_m32).plot(showlnl=True,ylim=ylim)
plt.gca().set_ylim(ylim)

fig.add_subplot(122)
SEDPlotter(sed_j2300).plot(showlnl=True,ylim=ylim)
plt.gca().set_ylim(ylim)
