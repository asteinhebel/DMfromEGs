...
project='dSphs'
run_id = ''
data = pd.read_csv('/zfs/astrohe/alexmcd/Stacking_Analysis/dSphs/Source_Data/dwarf_sample_full.csv')

#array_path should point to where you have the ouotput of the diMauro_likelihood.py script
array_path = '/zfs/astrohe/{}/Stacking_Analysis/{}/Numpy_Arrays/diMauro/{}/'\
...
for i, srcname in enumerate(srclist):
    ...
    try:
      like_file = np.load(array_path+'/{}/{}_noprior_freeBG_dlike_{}_{}EW.npy'.format(srcname,srcname,field,channelname, EWcorr))
      like_file_prior = np.load(array_path+'/{}/{}_Jprior_freeBG_dlike_{}_{}EW.npy'.format(srcname,srcname,field,channelname, EWcorr))
    except:
      print("{} Does not exist".format())

    # subtarcting the like_file[0,0] is to ensure that the 
    # low cross section values have a delta-loglikelihood of 0
    # This is a correction I added since I notcied some targets had an 
...
plotTS(summed_noprior.T, mass_vec, sigmav_vec, vmin=-3,)
plotTS(summed_wprior.T, mass_vec, sigmav_vec, vmin=-3,)
...
plt.plot(mass_vec, sigmavUL_noprior, color='k',   label='no Prior')
...
#plt.savefig('plots/blank_band_comp.png', facecolor='white', transparent=False)
